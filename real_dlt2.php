<?php
function real_dlt2($configKey, $configValue)
{
    require 'config_positions.php';
    // require 'config_positions.php';
    $url = 'http://gpsservice.dlt.go.th/gps/add/locations';
    $apiKey = "mirada:0S7ldi";
    ...
    FROM
    `devices`
    INNER JOIN `positions` ON `positions`.`deviceid` = `devices`.`id` WHERE `devices`.`connect_dlt2` = 1 AND `positions`.`id` > $configValue LIMIT 9000";
