<?php
function real_dlt($configKey, $configValue)
{
    ...
    FROM
    `devices`
    INNER JOIN `positions` ON `positions`.`deviceid` = `devices`.`id` WHERE `devices`.`connect` = 1 AND `positions`.`id` > $configValue LIMIT 9000";
